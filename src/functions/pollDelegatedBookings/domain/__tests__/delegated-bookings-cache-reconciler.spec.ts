...
import {
  compressedMockNewDelegatedExaminerTestSlot,
  compressedMockOldDelegatedExaminerTestSlot,
} from '../__mocks__/delegated-examiner-test-slot.mock';
...
describe('Delegated booking cache reconciler', () => {
  ...
  beforeEach(() => {
    ...
    spyOn(cachedDelegatedBookingRepository, 'cacheDelegatedBookingDetails')
      .and.callFake(moqCacheBookings.object);
    spyOn(cachedDelegatedBookingRepository, 'unCacheDelegatedBookingDetails')
      .and.callFake(moqUncacheBookings.object);
  });
  ...
  describe('reconcileActiveAndCachedDelegatedBookings', () => {
    it('should issue writes to the cache for active bookings not already cached', async () => {
      ...
      await reconcileActiveAndCachedDelegatedBookings(
        ...
        new DateTime('2021-01-06T07:51:00')
      );
    ...
    it('should cache active bookings not already in the cache' +
        'and uncache those that are cached but not active', async () => {
      const activeBookings = [new DelegatedBookingDetail(
        12345678910,
        '123456',
        compressedMockOldDelegatedExaminerTestSlot
      )];
