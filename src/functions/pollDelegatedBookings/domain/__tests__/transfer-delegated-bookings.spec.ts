...
describe('transferDelegatedBookings module', () => {
  ...
  beforeEach(() => {
    ...
    spyOn(delegatedBookingsRepository, 'getActiveDelegatedExaminerBookings').and.callFake(moqDelBookingsRepo.object);
    spyOn(cachedDelegatedBookingsRepository, 'getCachedDelegatedExaminerBookings').and.callFake(
      moqCachedDelBookingRepo.object
    );
    spyOn(delegatedBookingsCacheReconciler, 'reconcileActiveAndCachedDelegatedBookings').and.callFake(
      moqReconciler.object
    );
  });
  describe('transferDelegatedBookings', () => {
    it(
      'should retrieve all the active examiners in the replica,' +
        'all the IDs in the cache and pass them to the reconciler',
      async () => {
        ...
        moqReconciler.verify(
          (x) => x(It.isValue(activeBookings), It.isValue(cachedBookingsDetails), It.isAny()),
          Times.once()
        );
      }
    );
