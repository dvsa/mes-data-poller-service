...
export const getActiveTestCentreExaminers = async (): Promise<TestCentreDetail[]> => {
  try {
    ...
    if (process.env.USE_MOCK_TARS_DATA === 'true') {
      info('Getting mock Test Centre Examiners');
      queryResult = await getMockTestCentreExaminers();
      info('got examiners', queryResult);
    } else {
...
const getTestCentreQuery = (): string => {
  const template = `
    SELECT E.INDIVIDUAL_ID                   AS INDIVIDUAL_ID,
           ...
           GROUP_CONCAT(DISTINCT TC.TC_COST_CENTRE_CODE) AS TEST_CENTRES,
           t3.STAFF_NUMBERS
    FROM EXAMINER AS E
           INNER JOIN
         (SELECT *
         ...
         ON E.INDIVIDUAL_ID = PS.INDIVIDUAL_ID
           INNER JOIN TEST_CENTRE AS TC
                      ON PS.TC_ID = TC.TC_ID
           INNER JOIN (
      ...
      FROM PROGRAMME_SLOT AS PS1
             INNER JOIN TEST_CENTRE AS TC1
                        ON PS1.TC_ID = TC1.TC_ID
             LEFT JOIN (
