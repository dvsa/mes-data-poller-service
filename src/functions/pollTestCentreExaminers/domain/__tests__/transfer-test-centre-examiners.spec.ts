...
describe('transferTestCentreExaminers', () => {
  ...
  beforeEach(() => {
    ...
    spyOn(testCentreRepository, 'getActiveTestCentreExaminers').and.callFake(moqTestCentreRepo.object);
    spyOn(cachedTestCentreRepository, 'getCachedTestCentreExaminers').and.callFake(moqCachedTestCentreRepo.object);
    spyOn(testCentreCacheReconciler, 'reconcileActiveAndCachedTestCentreRows').and.callFake(moqReconciler.object);
  });
  describe('transferTestCentreExaminers', () => {
    it(
      'should retrieve all the active rows in the replica, ' +
        'all the staffNums in the cache and pass them to the reconciler',
      async () => {
